...
- Choice: define and complete `stage-f-18`, rebuild `/app` into a denser project start surface with top-level login or session actions, one lightweight guided-demo row, one manual-create surface, and one bounded existing-work region; rebuild the main workspace so the left side reads as a research workflow with clickable prompt chips, an obvious `闁诲孩顔栭崰鎺楀磻閹炬枼鏀芥い鏃傗拡閸庡繘鏌涢妸銉╁弰鐎殿喗鎮傞獮?CTA, and visible analysis progress in the main column; archive the task; and return Stage F to final closeout review instead of extending the stage again by default.
...
- Choice: replace the older generic Research product definition with `AI 鍓嶆部鐮旂┒`, define it as a workflow that starts
...
- Impact: future MCP work, product framing, and workflow design should treat `AI 鍓嶆部鐮旂┒` as the reference module
...
- Context: after `stage-j-03`, the project had one independent MCP server, but the visible product path still needed to
  show the difference between MCP resource, MCP tool, and MCP prompt behavior inside `AI 鍓嶆部鐮旂┒`.
- Choice: complete `stage-j-04`, update the visible `AI 鍓嶆部鐮旂┒` MCP path so it uses one MCP resource for digest
...
- Context: `stage-j-04` made resource, tool, and prompt behavior visible on the same `AI 鍓嶆部鐮旂┒` path, but the
...
- Choice: complete `stage-j-05`, extend run traces and regression review to include MCP tool and prompt identity,
  improve the operator-facing review surface, archive the task, and write a short Stage J learning summary in `AI 鍓嶆部鐮旂┒`
  terms.
...
- Context: all Stage J tasks are now finished: the MCP capability map is fixed, the independent server exists outside
  this repo, the visible `AI 鍓嶆部鐮旂┒` path uses MCP resource, tool, and prompt distinctly, and auth plus review signals
...
- Why: the next bounded goal is product coherence around `AI 鍓嶆部鐮旂┒`, not more protocol learning for its own sake.
...
- Context: after opening Stage K, the repo needed one bounded execution wave that could turn the completed MCP baseline
  into a coherent `AI 鍓嶆部鐮旂┒` module without reopening broad capability work.
...
- Context: Stage J delivered a coherent MCP baseline, but the visible product surface still depended on broad generic
  Research wording and did not yet expose one durable `AI 鍓嶆部鐮旂┒` result contract.
- Choice: complete `stage-k-02`, replace the older generic Research wording with one explicit `AI 鍓嶆部鐮旂┒` object
...
- Why: productization needed a stable module definition before source intake, output restructuring, and surface work
  could stay coherent.
- Impact: `AI 鍓嶆部鐮旂┒` is now the explicit reference workflow and later Stage K work can build on one narrower domain
...
- Context: after `stage-k-02`, the module definition was narrower, but the product still lacked one durable record for
  `AI 鍓嶆部鐮旂┒` work and one explicit persistence path for frontier outputs beyond raw traces or transient chat turns.
...
- Why: `AI 鍓嶆部鐮旂┒` should create durable research records instead of behaving like one more chat-only surface.
...
- Why: the module should expose one coherent output grammar that matches the new `AI 鍓嶆部鐮旂┒` contract.
...
- Context: after `stage-k-04`, the repo had the right records and outputs, but the visible workbench still needed to
  stop behaving like a generic Research shell and start presenting `AI 鍓嶆部鐮旂┒` as its own concrete module surface.
- Choice: complete `stage-k-05`, reframe the visible workbench around `AI 鍓嶆部鐮旂┒` outputs and records, archive the
...
- Context: Stage K delivered the right records and outputs, but the visible `AI 鍓嶆部鐮旂┒` workbench still exposed too much technical process, including a right rail, explicit analysis-mode switching, a separate trace or readiness entry, and a separate formal-output action surface that duplicated the main result path.
- Choice: keep the default `AI 鍓嶆部鐮旂┒` workbench to one lighter single-column path built around source intake, one current research result, and recent research records; remove the right rail from that module; stop exposing trace or readiness and separate formal-output actions in the main path; and hide analysis-mode switching from ordinary users.
...
- Context: the Stage K wave is now fully complete: the repo has one explicit `AI 鍓嶆部鐮旂┒` contract, one durable
...
- Why: the bounded Stage K goal was to productize `AI 鍓嶆部鐮旂┒`, and that goal is now met.
...
- Context: the old `闇€瑕佸厛鐧诲綍` card was still appearing on protected workspace and module pages, and it now reads like a leftover internal surface rather than a necessary product state.
...
- Choice: keep the legacy backend `/auth/login` and `/auth/register` APIs for compatibility, add one new `/auth/enter` API for the unified path, reduce the frontend to one `/login` surface with only `璐﹀彿` and `瀵嗙爜`, and redirect `/register` into that same page instead of preserving a second auth UI.
...
- ID: DEC-2026-04-24-151
- Date: 2026-04-24
- Status: Confirmed
- Source: Human + Implementation
- Topic: add one fixed AI hot tracker replay corpus and judgment-calibration suite for offline inspection
- Context: the hot-tracker product loop already had runtime truth, machine-readable evaluation checks, and a cleaner brief contract, but judgment quality still had to be inferred from live runs and a handful of isolated tests. That made it too easy to change scoring, clustering, delta, or event-memory logic without a stable calibration baseline.
- Choice: add one internal replay corpus that replays fixed official, media, research, and open-source scenarios against the current decision layer, and make that corpus assert ranking priority, conservative same-event clustering, steady-state suppression, threshold-driven notify, and replacement-memory behavior.
- Why: the first module now needs a durable quality loop, not just a consumer surface. A fixed replay suite gives the repo one stable way to inspect whether the hot-tracker agent still judges changes the way the product intends.
- Impact: future tuning of hot-tracker scoring, clustering, delta, or signal-memory behavior can now be checked against one deterministic offline suite before someone relies on live runs to notice regressions.
- Related Task: `tasks/archive/ai-hot-tracker-replay-corpus-and-judgment-calibration.md`
- Supersedes:

## Decision Entry

- ID: DEC-2026-04-24-152
- Date: 2026-04-24
- Status: Confirmed
- Source: Human + Implementation
- Topic: expose AI hot tracker replay calibration on the internal evaluation surface
- Context: the hot-tracker backend already had a fixed replay corpus and deterministic judgment-calibration suite, but that quality loop still lived only in tests and internal service code. The product-side internal evaluation view could explain a single saved run, yet it could not show whether the broader decision layer was still calibrated.
- Choice: add one authenticated replay-evaluation endpoint and read it only from the hidden ?view=evaluation path, so internal reviewers can inspect replay-suite status, case coverage, and failing findings without exposing that calibration surface on the consumer path.
...
- Choice: keep full source and scoring data in backend records for references and evaluation, but send only a compact cluster payload into the brief-synthesis model call: the top four report candidates, short summaries, bounded rank reasons, limited supporting items, and no URL or score-breakdown debug fields.
...
- Impact: hot-tracker report generation is less likely to exceed the runtime budget on Qwen JSON-mode calls, while reference-source rendering and internal evaluation still read the complete stored source items, candidate set, and decision metadata.
- Related Task: `tasks/archive/ai-hot-tracker-report-generation-payload-fix.md`
- Supersedes:

## Decision Entry

- ID: DEC-2026-06-30-163
- Date: 2026-06-30
- Status: Confirmed
- Source: Human + Implementation
