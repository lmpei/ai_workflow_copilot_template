...
def test_tool_assisted_chat_plans_search_executes_tools_and_returns_sources(monkeypatch) -> None:
    ...
    prior_memory = ResearchRunMemoryContext(
        source_run_id="run-1",
        summary="The previous pass found pricing pressure but needed stronger grounding.",
        evidence_state="no_grounded_matches",
        recommended_next_step="Refine the question around pricing pressure.",
    ...
    assert result.tool_steps[0].detail == prior_memory.summary
    ...
    assert "Bounded prior run memory" in fake_interface.json_calls[0]["messages"][1].content
    assert prior_memory.summary in fake_interface.text_calls[0]["messages"][1].content
...
def test_tool_assisted_chat_returns_guidance_when_no_documents(monkeypatch) -> None:
    ...
    assert "No usable research material is connected yet" in result.answer
