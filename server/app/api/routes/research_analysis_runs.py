...
from app.services.ai_hot_tracker_follow_up_service import answer_ai_hot_tracker_follow_up
from app.services.ai_hot_tracker_report_service import (
    AiHotTrackerReportAccessError,
    AiHotTrackerReportGenerationError,
    generate_ai_hot_tracker_report,
)
...
@router.post(
    ...
    response_model=AiHotTrackerReportResponse,
)
async def generate_hot_tracker_report(
    workspace_id: str,
    current_user: User = Depends(get_current_user),
) -> AiHotTrackerReportResponse:
    try:
        return generate_ai_hot_tracker_report(
            workspace_id=workspace_id,
            user_id=current_user.id,
        )
    except AiHotTrackerReportAccessError as error:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=str(error)) from error
    except AiHotTrackerReportGenerationError as error:
...
@router.post(
    "/workspaces/{workspace_id}/ai-hot-tracker/follow-up",
    response_model=AiHotTrackerFollowUpResponse,
)
async def follow_up_ai_hot_tracker(
    workspace_id: str,
    payload: AiHotTrackerFollowUpRequest,
    current_user: User = Depends(get_current_user),
) -> AiHotTrackerFollowUpResponse:
    try:
        return answer_ai_hot_tracker_follow_up(payload)
